...
- **install-standard 合规收口（v2.5.1 守卫的同批补课）**：① `installByRelation()` 原用官方模板的
  目录拷贝语法（overwrite=true，标准 §三/§五 明令禁止，`install_standard` 审计规则会 FAIL）——改为
  自实现 `copyDir()` / `copyMissingFiles()`，落盘口径统一为「**目标已存在即只补缺失文件，不看
  `$isFirst`**」（旧覆盖语义会静默抹掉宿主定制）；② 签名补 `$isFirst`（标准要求，默认 true 保持调用点兼容），
  `install()`/`update()` 分别传 `$isFirst` / `false`；③ 源目录不存在时明确 `continue`（旧实现对着不存在的
  目录调拷贝，属空操作却会打印误导性的 "Create …"）；④ 删死代码 `removeDir()`（守卫接管后无调用者）。
- **清单基准修正（本次自测抓到的真 bug）**：本包接线配置随源码落 `src/config/...`（与其它包「配置在包根」
  不同），而 `syncManifest()` 按统一布局取了 `dirname(__DIR__)` ⇒ 源目录找不到、**清单写空**、卸载退化成
  「全部保留」（保守方向，不会误删，但包文件永不清理）。改为与 `installByRelation()` 同基准 `__DIR__`。
- **验证（沙箱 6 断言）**：宿主自建 `migrate.php` 全程未被碰；安装补齐包内 2 个缺失文件；清单只登记这 2 个
  包内一致文件（宿主文件不入单）；卸载精确删除这 2 个、保留宿主文件。
...
Phinx 按版本升序执行，占位未来日期版本会排到全部真实日期迁移之后——全新空库上
真实日期的业务种子先跑而依赖的表后建，直接断链（2026-09-16 全工作区 65 个手编
`202612xx000000` 类文件根治实案，TASK-20260916-544/TASK-20260914-355）。预检扫描
本次通道装载的全部迁移文件，版本号 > 当前时间 + 5 分钟（时钟偏移余量）即中止并列出
文件与修复指引；与 v2.3.0 撞号 / v2.4.0 命名形态同规格。
